...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    craftsman_id BIGINT NOT NULL, -- идентификатор самого популярного мастера ручной работы у заказчика. Если заказчик сделал одинаковое количество заказов у нескольких мастеров, возьмите любого;
...
WITH --создать в представлении dwh_delta, dwh_update_delta, dwh_delta_insert_result, dwh_delta_update_result, insert_delta, update_delta, insert_load_date
...
insert_load_date AS ( -- делаем запись в таблицу загрузок о том, когда была совершена загрузка, чтобы в следующий раз взять данные,
...
select * from dwh.customer_report_datamart order by customer_money desc limit 20;