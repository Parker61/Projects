...
def get_report(ti):
    ...
    if not report_id:
        raise TimeoutError('Report is empty')

    ti.xcom_push(key='report_id', value=report_id)
    print(f'Report_id={report_id}')


def get_increment(date, ti):  # business_dt
    print('Making request get_increment')
    report_id = ti.xcom_pull(key='report_id')
    response = requests.get(f'{base_url}/'
                            f'get_increment?report_id={report_id}'
...
def upload_data_to_staging(filename, date, pg_table, pg_schema, ti):
    ...
    s3_filename = (f'https://storage.yandexcloud.net/s3-sprint3/cohort_'
                   f'{cohort}/{nickname}/project/{increment_id}/{filename}')
    print(s3_filename)
    ...
    #  изменения значений payment_amount на отрицательные для refunded
    # if 'payment_amount' in df.columns:
    # df['payment_amount'] = df.apply(
    # lambda row: -row['payment_amount'] if row['status'] == 'refunded'
    # else row['payment_amount'], axis=1)
...
with DAG(
        ...
        catchup=True,
        start_date=datetime.today() - timedelta(days=7),  # первый запуск
        end_date=datetime.today() - timedelta(days=0),  # и седьмой вчера
) as dag:
